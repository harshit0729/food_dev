...
exports.createOrder = asyncHandler(async (req, res, next) => {
  ...
  for (const item of cart.items) {
    await FoodItem.findByIdAndUpdate(item.foodItem._id, { $inc: { orderCount: item.quantity } });
  }
  await Restaurant.findByIdAndUpdate(cart.restaurant, { $inc: { orderCount: 1 } });
...
exports.getOrders = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json({
    ...
    totalPages: Math.ceil(total / limit), currentPage: parseInt(page), data: orders,
  });
});
...
exports.updateOrderStatus = asyncHandler(async (req, res, next) => {
  ...
  if (status === 'delivered') { order.deliveredAt = Date.now(); order.paymentStatus = 'completed'; }
  if (status === 'cancelled' && order.paymentStatus === 'completed') order.paymentStatus = 'refunded';
  await order.save();
  res.status(200).json({ success: true, data: order });
});
...
exports.cancelOrder = asyncHandler(async (req, res, next) => {
  const order = await Order.findById(req.params.id);
  if (!order) return next(new ErrorResponse('Order not found', 404));
  if (order.user.toString() !== req.user.id) return next(new ErrorResponse('Not authorized to cancel this order', 403));
  if (!['pending', 'confirmed'].includes(order.status)) return next(new ErrorResponse('Order cannot be cancelled at this stage', 400));
