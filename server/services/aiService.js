...
exports.getRecommendations = async (query, type = 'mood', userId = null) => {
  const systemPrompts = {
    mood: `You are a food recommendation expert. Suggest 5 food items based on the user's mood.
      Return only the food names, one per line. Be creative and specific.`,

    weather: `You are a food recommendation expert. Suggest 5 food items suitable for the given weather condition.
      Return only the food names, one per line.`,

    budget: `You are a food recommendation expert. Suggest 5 budget-friendly food items based on the user's budget.
      Return only the food names, one per line.`,

    healthy: `You are a nutrition expert. Suggest 5 healthy meal options based on the user's request.
      Return only the food names, one per line.`,

    trending: `You are a food trend analyst. Suggest 5 trending food items people are ordering right now.
      Return only the food names, one per line.`,

    meal_plan: `You are a nutritionist and meal planner. Create a personalized meal plan based on user's goal.
      Return breakfast, lunch, dinner, and snacks suggestions. Format clearly.`,

    search: `You are a smart food search assistant. Interpret the user's natural language query and suggest
      relevant food items they might be looking for. Return food names one per line.`,

    chat: `You are a friendly food assistant chatbot. Help users with food suggestions, restaurant recommendations,
      order help, and FAQs. Be concise and helpful.`,

    summarize: `You are a review analyst. Summarize the following food/restaurant reviews into:
      1. Positive Summary (key positive points)
      2. Negative Summary (key negative points)
  ...
  if (!aiText) {
    return getFallbackRecommendations(query, type);
  }

  const result = type === 'summarize'
    ? aiText
  ...
  if (userId && type !== 'chat' && type !== 'summarize') {
    await AIRecommendation.create({
      ...
      query,
      response: result,
      recommendations: result.map((name) => ({ name })),
    });
  }
...
const getFallbackRecommendations = (query, type) => {
  const fallbacks = {
    mood: {
      happy: ['Chocolate Cake', 'Ice Cream Sundae', 'Pizza', 'Sushi', 'Fruit Smoothie'],
      sad: ['Mac and Cheese', 'Warm Soup', 'Dark Chocolate', 'Mashed Potatoes', 'Hot Tea'],
      stressed: ['Green Salad', 'Avocado Toast', 'Smoothie Bowl', 'Herbal Tea', 'Dark Chocolate'],
      tired: ['Coffee', 'Energy Smoothie', 'Protein Bar', 'Banana', 'Oatmeal'],
      default: ['Biryani', 'Pizza', 'Burger', 'Noodles', 'Ice Cream'],
    },
    weather: {
      hot: ['Ice Cream', 'Cold Coffee', 'Watermelon Juice', 'Salad', 'Smoothie'],
      cold: ['Hot Soup', 'Hot Chocolate', 'Stew', 'Ginger Tea', 'Warm Pasta'],
      rainy: ['Pakoras', 'Garam Chai', 'Soup', 'Corn on Cob', 'Noodles'],
      default: ['Lemonade', 'Iced Tea', 'Fresh Juice', 'Fruit Salad', 'Yogurt'],
    },
    budget: {
      low: ['Street Food', 'Local Thali', 'Sandwich', 'Noodles', 'Rice Bowl'],
      medium: ['Biryani', 'Pizza', 'Pasta', 'Burger', 'Curry'],
      high: ['Steak', 'Sushi', 'Lobster', 'Fine Dining Platter', 'Gourmet Pizza'],
      default: ['Burger', 'Pizza', 'Sandwich', 'Pasta', 'Salad'],
    },
    healthy: {
      default: ['Greek Salad', 'Grilled Chicken', 'Quinoa Bowl', 'Smoothie', 'Fruit Bowl'],
    },
    trending: {
      default: ['Sushi Bowl', 'Avocado Toast', 'Bubble Tea', 'Beyond Burger', 'Keto Bowl'],
    },
  };
...
exports.getMealPlan = async (userGoal, preferences = {}) => {
  const prompt = `Create a detailed daily meal plan for a user with goal: "${userGoal}".
    ${preferences.dietary ? `Dietary preference: ${preferences.dietary}` : ''}
    ${preferences.calories ? `Target calories: ${preferences.calories}` : ''}
    Include breakfast, lunch, dinner, and 2 snacks. Format with clear sections.`;

  const result = await exports.getRecommendations(prompt, 'meal_plan');
  if (Array.isArray(result) && result.length <= 5) {
    return formatMealPlanFallback(userGoal);
  }
...
Dinner: Grled fish/tofu with quinoa and roasted vegetables`,
  };
